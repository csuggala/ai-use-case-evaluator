...
- EU AI Act${isEU ? ' (APPLIES — EU/Global users detected)' : ' (lower priority — no EU users detected)'}: Unacceptable Risk=prohibited; High Risk=recruitment/credit/healthcare diagnostics/critical infrastructure/education — requires logging, human oversight, conformity assessment; Limited Risk=chatbots/emotion recognition — transparency disclosures; Minimal Risk=standard obligations
- ${isUS ? 'US AI Laws (APPLIES):' : 'US AI Laws:'} California CCPA Automated Decision-Making (consumer decisions→notice+opt-out); California AB 2013/SB 942 (generative AI→training data docs+watermarking); Colorado AI Act SB 189 (housing/employment/education/banking decisions→bias impact assessment); Illinois BIPA (biometric data→consent)
`;

    // Always include core data privacy
    regulatoryFrameworks += `
DATA PRIVACY (apply based on geography):
- ${isEU ? 'GDPR (APPLIES — EU users detected):' : 'GDPR:'} right to be forgotten, purpose limitation, lawful basis, DPA notification
- ${isCA ? 'CCPA/CPRA (APPLIES — California/Global detected):' : 'CCPA/CPRA:'} data minimization, LLM vendor sharing disclosure
`;
    ...
    if (isEducation) regulatoryFrameworks += `\nEDUCATION (APPLIES): FERPA (student educational records — strict consent for AI), COPPA (students under 13), EU AI Act High Risk (AI in educational assessment), Student Data Privacy Consortium agreements.\n`;
    ...
    if (isChildren) regulatoryFrameworks += `\nCHILDREN'S APPS (APPLIES): COPPA (mandatory for users under 13 — prohibits behavioral tracking, requires parental consent), KOSA (Kids Online Safety Act), UK Age Appropriate Design Code, EU AI Act heightened scrutiny for child-facing AI.\n`;
    ...
    if (!isHealthcare && !isFinance && !isInsurance && !isLegal && !isEducation && !isRetail && !isTech && !isManufacturing && !isGovt && !isHR && !isRealEstate && !isPharma && !isEnergy && !isTelecom && !isDefense && !isNonProfit && !isChildren) {
      regulatoryFrameworks += `\nINDUSTRY: No specific sector detected — apply horizontal AI and data privacy frameworks only. Flag any sector-specific regulations identified from use case context.\n`;
    }
...

{
  ...
  "composite_score": 2.1,
  "composite_score_rationale": "1 sentence — this is the primary ranking metric",
...
- input_quality.score_confidence: reflects how much the vagueness of inputs affects reliability of scores — High/Medium/Low
- input_quality.flags: max 2 items — only flag genuinely problematic vagueness, not nitpicks
- regulatory_requirements: max 4 items — most important only
...
- All text fields: 1 sentence maximum — be concise
